...
```bash
${CLAUDE_SKILL_ROOT}/scripts/validate_skill.sh <path/to/skill-directory>
```

The script validates the skill using pi itself: a headless agent checks frontmatter format, required fields, naming rules, description quality, and structure against the Agent Skills specification, using its tools for precise measurements. It exits 0 on PASS and 1 on FAIL. Fix any errors and re-run until validation passes.
...
### Validation
- [ ] `${CLAUDE_SKILL_ROOT}/scripts/validate_skill.sh` passes
