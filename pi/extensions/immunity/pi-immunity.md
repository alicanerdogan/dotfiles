...
6. **Outside-cwd boundary (implicit, no config knob)**: a target whose realpath lies outside the project cwd — including `..` traversal and symlink escapes — prompts. Precedence: protected rules > explicit allows > boundary > allow. Users steer the boundary with the existing fields only: an `allowed` entry whitelists a specific outside path, a `protected` rule gates a path anywhere.
...
Type.Object({
  ...
  paths:   Type.Optional(Type.Object({
    blocked: Type.Array(Type.String({ description: "concrete paths the command touches — the user may protect or allow each" })),
  })),
  commands: Type.Optional(Type.Object({
    blocked: Type.Array(Type.Object({
      raw:     Type.String({ description: "the exact command to block" }),
      general: Type.Optional(Type.String({ description: "broader pattern (audit-only, never applied automatically)" })),
    })),
  })),
})
```

- `paths.blocked` / `commands.blocked` are the model's **actionable suggestions** (2025-08-06): the prompt menu renders per-path `Protect`/`Allow` options (project or global scope) and per-command `Block <raw>` options. Only the user's menu choice writes a rule — `raw` is trusted verbatim, `general` is audit-only. Picking *Allow* on a path writes `pathnames.allowed` and **retries the pipeline once** so the model sees the updated policy (precedence: explicit allowances override the general policy); if the retry still rejects, the prompt reopens (max 2 retries, then the user's override is honored). Suggestion-driven writes are audited as `event: "rule"` lines, and the verdict trail stores the suggestions as returned.
...
- *Actionable LLM suggestions (2025-08-06):* the verdict may carry `paths.blocked` (concrete paths) and `commands.blocked` (`raw` + audit-only `general`). The prompt menu renders per-path `Protect`/`Allow` (project/global) and per-command `Block <raw>` options; only the user's choice writes a rule (protect → `pathnames.protected`, allow → `pathnames.allowed`, block → `autoDenyPatterns`), `raw` is trusted verbatim, `general` never applies itself. Allow-path writes re-run the pipeline once (the model must see the updated policy — the analysis message carries the effective policy and explicit allowances take precedence over the general rules); retries cap at 2, then the override is honored. User choices are audited (`event: "rule"` lines) alongside the verdict trail, which stores the suggestions as returned. No new config fields — suggestions are transient, rules live in the existing sections.
- *Outside-cwd boundary (2025-08-06, amended 2025-08-06):* file-tool access whose **realpath** lies outside the project cwd — `..` traversal and symlink escapes caught via realResolve (nearest-existing-ancestor realpath, never throws) — **prompts by default**. No config knob (user feedback: reuse the existing fields): `protected` rules fire anywhere, `allowed` entries (and grants) bypass the boundary, precedence protected > allowed > boundary > allow. On the LLM side the analysis message carries `Project cwd` plus the effective **path policy** (compact `protected`/`allowed` lists) so the model aligns verdicts with the user's rules (e.g. `cat .env` is flagged when `**/.env*` is protected); the default policy denies creating/modifying/reading outside cwd **unless the Policy line explicitly allows the path** (no automatic system-config exception — /etc/hosts reads DENY and can be allowlisted via the suggestion flow). Files stay deterministic-only (no LLM), so the boundary is the file gate.
