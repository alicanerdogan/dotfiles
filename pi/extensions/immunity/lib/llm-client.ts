...
export const DEFAULT_SYSTEM_PROMPT =
  ...
  "- When you deny or flag a command, populate the suggestion fields so the user can act: paths.blocked = concrete blocked paths the command touches (protect or allow them) — always normalized concrete paths, never globs or wildcards (*, **, ?); commands.blocked = the exact blocked command as run (raw) plus optionally a broader command form (general) the same rule could cover — general must itself be a command (e.g. `find ~/repos -maxdepth 1 -type d | wc -l` for a variant with different flags), never a prose description; omit general when you cannot produce a command form. general is only informational — never apply it yourself.\n" +
